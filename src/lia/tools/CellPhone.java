...
public class CellPhone extends JFrame {
	...
	private class PhoneButtonAction extends AbstractAction {
		...
		public PhoneButtonAction(char key, String name) {
			super("<html><center><b>" + key + "</b><br>" + name
					+ "</center></html>");
			this.key = key;
		}
		...
		public void actionPerformed(ActionEvent event) {
			...
			case '1':
				curWord++;
				if (words == null || curWord >= words.length)
					curWord = 0;
				message = "";
				break;
	...
	private long findWords() {
		...
		try {
			Date start = new Date();
			Hits hits = searcher.search(query, new Sort(new SortField[] {
					SortField.FIELD_SCORE,
					new SortField("length", SortField.INT),
					new SortField("word") }));
			Date end = new Date();
			...
			if (debug) {
				if (hits.length() > 0) {
					System.out.println(number + ": " + hits.doc(0).get("word")
							+ " (" + hits.length() + ")");
				}
			}
		} catch (IOException e) {
	...
	public static void main(String[] args) throws IOException {
		...
		CellPhone app = new CellPhone(new RAMDirectory(
				FSDirectory.getDirectory(args[0], false)));
