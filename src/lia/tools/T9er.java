...
public class T9er {
	private static String[] keys = { "2abc", "3def", "4ghi", "5jkl", "6mno",
			"7pqrs", "8tuv", "9wxyz" };
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < keys.length; i++) {
			...
			for (int j = 1; j < key.length(); j++) {
				keyMap.put(new Character(key.charAt(j)),
						new Character(key.charAt(0)));
				System.out.println(new Character(key.charAt(j)) + " = "
						+ new Character(key.charAt(0)));
			}
		...
		IndexWriter writer = new IndexWriter(t9dir, new WhitespaceAnalyzer(),
				true);
		...
		for (int i = 0; i < numDocs; i++) {
			...
			newDoc.add(new Field("length", Integer.toString(word.length()),
					false, true, false));
	...
	private static String t9(String word) {
		...
		for (int i = 0; i < word.length(); i++) {
			t9[i] = ((Character) keyMap.get(new Character(word.charAt(i))))
					.charValue();
		}
