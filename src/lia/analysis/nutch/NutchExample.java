...
public class NutchExample {
	public static void main(String[] args) throws IOException {
		...
		net.nutch.searcher.Query nutchQuery = net.nutch.searcher.Query
				.parse("\"the quick brown\"");
		Query query = QueryTranslator.translate(nutchQuery);
		System.out.println("query = " + query);
	}

	/**
	 * Copy of AnalyzerUtils.displayTokensWithPositions, except uses the
	 * "content" field instead of "contents". Nutch demands "content".
	 */
	private static void displayTokensWithDetails(Analyzer analyzer, String text)
			throws IOException {
		...
		for (int i = 0; i < tokens.length; i++) {
			...
			System.out
					.print("[" + token.termText() + ":" + token.type() + "] ");
		}
		System.out.println();
	}

	/**
	 * Copy of AnalyzerUtils.tokensFromAnalysis, except uses the "content" field
	 * instead of "contents". Nutch demands "content".
	 */
