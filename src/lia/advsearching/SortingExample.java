...
public class SortingExample {
	...
	public void displayHits(Query query, Sort sort) throws IOException {
		...
		System.out.println("\nResults for: " + query.toString() + " sorted by "
				+ sort);
		...
		for (int i = 0; i < hits.length(); i++) {
			...
			System.out.println(StringUtils.rightPad(
					...
					+ StringUtils.leftPad(scoreFormatter.format(hits.score(i)),
							12));
	...
	public static void main(String[] args) throws Exception {
		...
		example.displayHits(allBooks, new Sort(new SortField[] {
				new SortField("category"), SortField.FIELD_SCORE,
				new SortField("pubmonth", SortField.INT, true) }));

		example.displayHits(allBooks, new Sort(new SortField[] {
				SortField.FIELD_SCORE, new SortField("category") }));
	}
}
